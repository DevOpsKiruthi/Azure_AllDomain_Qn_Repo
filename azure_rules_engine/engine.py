...
class ExactFormatAzureRulesEngine:
    ...
    def generate_policy(self, services: List[str], scenario: str = "") -> Dict:
        """Generate comprehensive policy using intelligent analysis"""
        
        # Import intelligent generator
        try:
            from .intelligent_policy_generator import intelligent_policy_generator
            # Use intelligent policy generation
            return intelligent_policy_generator.generate_comprehensive_policy(services, scenario)
        except Exception as e:
            # Log the error for debugging
            print(f"⚠️  Intelligent policy generator failed: {e}")
            # Fallback to basic policy generation
            return self._generate_basic_policy(services)
    
    def _generate_basic_policy(self, services: List[str]) -> Dict:
        """Fallback basic policy generation"""
        if not services:
            return {}
        ...
        # For multiple services - combination format
        else:
            return {
                "if": {
                    "allOf": [
    ...
    def generate_validation_script(self, service) -> str:
        """Generate validation in USER'S EXACT FORMAT - no push statements!"""
        
        # Handle if service is passed as a list
        if isinstance(service, list):
            service = service[0] if service else "Unknown"
...
(async () => {{
    ...
    def services(self) -> List[str]:
        """Property for compatibility with agent"""
